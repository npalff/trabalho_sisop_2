...
//Transmite uma mensagem de elei��o para todos os processos com id maior
...
// Processar uma mensagem de elei��o
void process_election(message msg) {
    ...
    // Verifica se o processo atual j� realizou uma elei��o
    if (!election_in_progress) {
        // Inicia uma nova elei��o
        election_in_progress = true;
        printf("Process %d starts a new election.\n", my_id);
        send_election_msg();
    } else {
        // Elei��o j� em andamento
    ...
    if (msg.from > my_id) {
        ...
        // Se o id do remetente for menor, n�o responde e inicia sua pr�pria elei��o
        printf("Process %d received an ELECTION message from process %d with a lower id. Starting a new election...\n", my_id, msg.from);
        election_in_progress = true;
        send_election_msg();
    } else {
        // Se o id do remetente for igual ao do processo atual, ele � o novo coordenador
...
void process_answer(message msg) {
    ...
    // Verifica se o processo atual est� em uma elei��o e se a mensagem � do coordenador atual
    if (election_in_progress && msg.from == coordinator_id) {
        // Atualiza o tempo limite para a pr�xima mensagem do coordenador
